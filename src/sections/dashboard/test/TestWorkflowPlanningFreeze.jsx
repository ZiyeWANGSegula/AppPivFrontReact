...
export default function TestWorkflowPlanningFreeze({ onNextStep, onBackStep, currentTest, stepName }) {
    ...
    const preFreezeStatus = currentTest[`ProjTest${stepName}`]['preFreezeStatus']
    const freezeStatus = currentTest[`ProjTest${stepName}`]['freezeStatus']

    useEffect(() => {

        const data = {
            id: currentTest.id,
            step: `PLANNING_FREEZE`,
    ...
    const sections = [
        {
            title: "Pre Freeze",
            entity: "preFreeze",
            stepDataKey: "preFreezeEstimAt",
            validateState: preFreezeStatus,
        },
        {
            title: "Freeze",
            entity: "freeze",
            stepDataKey: "freezeEstimAt",
            validateState: freezeStatus,
        },
    ];
            ...
            <Card sx={{ width: '100%' }}>
                ...
                >
                    {sections.map((section, index) => (
                        <TestStatuFieldsSectionPlanningFreeze
                            title={section.title}
                ...
                >
                    ...
                    {sectionValidationStates.every((state) => state) && (
                        <Button
                            ...
                            sx={{ margin: 3 }}
                        >
                            {validateState ? 'Cancel validation' : 'Validate Planning Freeze'}
                        </Button>
                    )}
