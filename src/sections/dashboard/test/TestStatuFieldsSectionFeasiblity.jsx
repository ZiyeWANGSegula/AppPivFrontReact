...
TestStatuFieldsSectionFeasiblity.propTypes = {
    title: PropTypes.string,
    estimatedDate: PropTypes.string,
    stepData: PropTypes.object
};

export default function TestStatuFieldsSectionFeasiblity({ title, stepData, entity, estimatedDate, testId, onValidateStateChange, stepName, stepValidated }) {
    const theme = useTheme();
    const { user } = useAuthContext();
    const { enqueueSnackbar } = useSnackbar();
    const [tabValue, setTabValue] = useState(0);
    const [entityId, setEntityId] = useState(stepData[`${entity}`]);
    ...
    const onSubmit = (data) => {
        data.step = `FEASIBILITY_${entity.toUpperCase()}`;
        data.id = testId
    ...
    useEffect(() => {
        const params = {
            ...
            field: `FEASIBILITY_${entity.toUpperCase()}`,
        };
        console.log('params');
        console.log(params);
        ...
        const getCommentParams = {
            ...
            testStatus: `FEASIBILITY`,
            testStatusField: `FEASIBILITY_${entity.toUpperCase()}`,
        };
        console.log('params');
        console.log(params);
    ...
    return (
        ...
        <Box sx={{ width: '100%', height: '100%', bgcolor: 'background.paper', display: 'flex', flexDirection: 'column' }}>
            ...
            <TabPanel value={tabValue} index={0}>
                ...
                <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
                    <Box
                        ...
                        <RHFTextField name="entityId" label={`${title}`} defaultValue={entityId} InputProps={validateState ? { readOnly: true } : { readOnly: false }} />
            ...
            <TabPanel value={tabValue} index={1}>
                <TestWorkflowDocumentTabPanelList stepName={`FEASIBILITY_${entity.toUpperCase()}`} stepData={stepData} model={`ProjTest${stepName}`} entity={`test${stepName}`} setTotal={handleDocumentTotalChange}></TestWorkflowDocumentTabPanelList>
            </TabPanel>
            ...
            <TabPanel value={tabValue} index={3}>
                <TestWorkflowCommentTabPanelList stepName='FEASIBILITY' stepData={stepData} entity={entity.toUpperCase()} setTotal={handleCommentTotalChange}></TestWorkflowCommentTabPanelList>
            </TabPanel>
